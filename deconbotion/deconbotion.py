...
def createBar():
    ...
    h3,s3,v3,h3_,s3_,v3_ = loadHSV('hsv_kuning')
    cv2.namedWindow("Kuning")
    cv2.createTrackbar("H min","Kuning",h3,255,nothing)
    cv2.createTrackbar("S min","Kuning",s3,255,nothing)
    cv2.createTrackbar("V min","Kuning",v3,255,nothing)
    cv2.createTrackbar("H max","Kuning",h3_,255,nothing)
    cv2.createTrackbar("S max","Kuning",s3_,255,nothing)
    cv2.createTrackbar("V max","Kuning",v3_,255,nothing)
    cv2.createTrackbar("Save","Kuning",0,1,nothing)
...
def main():
    ...
    while True:
        ...
        lower_kuning = np.array([getBar("H min","Kuning"), getBar("S min","Kuning"), getBar("V min","Kuning")], dtype="uint8")
        upper_kuning = np.array([getBar("H max","Kuning"), getBar("S max","Kuning"), getBar("V max","Kuning")], dtype="uint8")

        #Enhance Image
        im_merah = enhance(hsv,lower_merah,upper_merah,gray)
        im_biru = enhance(hsv,lower_biru,upper_biru,gray)
        im_kuning = enhance(hsv,lower_kuning,upper_kuning,gray)

        #findContour for getting x , y and radius
        x_merah,y_merah,radius_merah = findContour(im_merah,"Merah")
        x_biru,y_biru,radius_biru = findContour(im_biru,"Biru")
        x_kuning,y_kuning,radius_kuning = findContour(im_kuning,"Biru")

        #Re-map value 0-20 scala
        kirimX_merah= map1(x_merah, 0, cap.get(3), 0, 20)
        kirimX_biru= map1(x_biru, 0, cap.get(3), 0, 20)
        kirimX_kuning= map1(x_kuning, 0, cap.get(3), 0, 20)

        #Kirim sinyal ke arduino sesuai dengan besar radius merah/ biru
        if(radius_merah >= 10):
            #Jika merah memenuhi (posisi kapal dakeat dermaga/finish) maka kirim ke fungsi arduino
            thread_serial = Arduino(kirimX=kirimX_merah,radius=radius_merah,warna="merah")
            print "Merah : X:",x_merah," Y:",y_merah," Radius:",radius_merah," kirmX:",kirimX_merah
        else:
            if(radius_biru>radius_kuning):
                #Jika biru memenuhi (posisi kapal berada di area rintangan biru) maka kirim ke fungsi arduino
                ...
                #Jika biru tidak memenuhi (posisi kapal berada di area rintangan kuning) maka kirim ke fungsi arduino
                thread_serial = Arduino(kirimX=kirimX_kuning,radius=radius_kuning,warna="kuning")
                print "Kuning : X:",x_kuning," Y:",y_kuning," Radius:",radius_kuning," kirmX:",kirimX_kuning
        ...
        if cv2.waitKey(1) & 0xFF == ord("q"):
            ...
            if(cv2.getTrackbarPos("Save","Kuning") == 1):
                dataHSV = np.concatenate((lower_kuning,upper_kuning))
                saveHSV("hsv_kuning",dataHSV)
